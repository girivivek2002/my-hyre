...
// ─── Mock Data ────────────────────────────────────────────────────────────────
const CHATS = [
  { id: 1, name: "Karri Saarinen", company: "Linear", lastMsg: "The technical review looks solid. Let's sync tomorrow.", time: "10:24 AM", unread: 2, avatar: "KS", match: 98, online: true },
  { id: 2, name: "Rauno Freiberg", company: "Vercel", lastMsg: "Did you have a chance to look at the motion spec?", time: "Wed", unread: 0, avatar: "RF", match: 94, online: false },
  { id: 3, name: "Mira Murati", company: "OpenAI", lastMsg: "AGI alignment is our top priority for this role.", time: "Tue", unread: 0, avatar: "MM", match: 89, online: true },
  { id: 4, name: "Sterling & Co. Hiring", company: "Recruitment", lastMsg: "Your offer letter is being drafted as we speak.", time: "Mon", unread: 0, avatar: "SC", match: 100, online: false },
];

const MESSAGES = [
  { id: 1, text: "Hi Karri, thanks for reaching out. I've reviewed the Linear Method docs.", sender: "me", time: "9:45 AM" },
  { id: 2, text: "Excellent. We're looking for someone who can bridge the gap between Figma and production code.", sender: "them", time: "10:12 AM" },
  { id: 3, text: "That's exactly my focus. I've been working on a unified design token system for my last three projects.", sender: "me", time: "10:15 AM" },
  { id: 4, text: "The technical review looks solid. Let's sync tomorrow at 10 AM PST to discuss the Design Systems Lead role further.", sender: "them", time: "10:24 AM" },
];
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#050505] text-slate-900 dark:text-white flex overflow-hidden selection:bg-blue-500/30 font-sans transition-colors duration-300">
      ...
      <motion.div initial={{ x: -100, opacity: 0 }} animate={{ x: 0, opacity: 1 }}
        ...
        <Image src="/logo.png" alt="Logo" width={32} height={32} className="mb-10 cursor-pointer" onClick={() => router.push("/candidate/dashboard")} />
        <div className="space-y-4">
          {[
            { icon: <LayoutDashboard size={22} />, path: "/candidate/dashboard" },
            { icon: <MessageSquare size={22} />, active: true },
            { icon: <Briefcase size={22} />, path: "/candidate/jobs" },
            { icon: <Settings size={22} />, path: "/candidate/settings" },
          ].map((item, i) => (
         ...
         <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-1">
            {CHATS.map((chat) => (
              <div key={chat.id} onClick={() => setActiveChat(chat)} className={`p-4 rounded-2xl cursor-pointer transition-all flex gap-4 relative group ${activeChat.id === chat.id ? 'bg-white dark:bg-neutral-900 shadow-xl' : 'hover:bg-slate-100/50 dark:hover:bg-neutral-900/40'}`}>
                 ...
                 {chat.unread > 0 && <div className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 bg-blue-500 rounded-full flex items-center justify-center text-[10px] text-white font-bold shadow-lg">{chat.unread}</div>}
              </div>
            ))}
         </div>
      </div>
         ...
         <div className="h-20 border-b border-slate-200 dark:border-neutral-800/60 px-8 flex justify-between items-center bg-white/40 dark:bg-neutral-950/40 shrink-0">
            <div className="flex items-center gap-4">
               ...
               <div>
                  ...
                  <p className="text-[10px] text-emerald-500 font-bold uppercase tracking-widest">{activeChat.online ? 'Active Now' : 'Last seen 2h ago'}</p>
               </div>
            </div>
         ...
         <div ref={scrollRef} className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
            {MESSAGES.map((msg) => (
              <motion.div key={msg.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`flex ${msg.sender === 'me' ? 'justify-end' : 'justify-start'}`}>
                 <div className={`max-w-[70%] space-y-2 ${msg.sender === 'me' ? 'items-end' : 'items-start'}`}>
                    <div className={`p-4 rounded-2xl text-sm leading-relaxed shadow-sm ${msg.sender === 'me' ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-white dark:bg-neutral-900 border border-slate-200 dark:border-neutral-800 rounded-tl-none'}`}>
                       {msg.text}
                    </div>
            ...
            <div className="flex justify-center py-4">
               <div className="px-4 py-2 rounded-full bg-blue-500/10 border border-blue-500/20 text-[10px] font-bold text-blue-500 uppercase tracking-widest flex items-center gap-2 animate-pulse">
                  <Sparkles size={12} fill="currentColor" /> Intelligence Insight Refined: Role fit increased to 98%
               </div>
            </div>
         </div>
         ...
         <div className="p-6 shrink-0">
            <GlassCard className="!rounded-[32px] overflow-visible">
               <div className="p-2 flex items-end gap-2">
                  ...
                  <textarea value={msgInput} onChange={(e) => setMsgInput(e.target.value)} placeholder="Craft your message..." rows={1} className="flex-1 bg-transparent py-3 px-2 text-sm focus:outline-none resize-none no-scrollbar font-medium" />
                  <button className="p-3 bg-blue-600 text-white rounded-2xl shadow-xl shadow-blue-500/20 hover:scale-105 active:scale-95 transition-all">
                     <Send size={20} />
                  </button>
               </div>
            </GlassCard>
      ...
      {/* Right Intelligence Pane (Context) */}
      <div className="hidden xl:flex w-80 border-l border-slate-200 dark:border-neutral-800/60 bg-white/30 dark:bg-neutral-950/30 backdrop-blur-md flex-col p-8 space-y-8 shrink-0 overflow-y-auto no-scrollbar">
         ...
         <div className="space-y-6">
            <div className="text-center">
               <div className="w-20 h-20 rounded-[30px] bg-gradient-to-br from-blue-600 to-indigo-700 flex items-center justify-center text-white text-2xl font-extrabold shadow-2xl mx-auto mb-4 relative overflow-hidden group">
                  {activeChat.avatar}
            ...
            <div className="flex justify-center gap-3">
               <div className="px-3 py-1 bg-blue-500/10 border border-blue-500/20 rounded-lg text-[10px] font-bold text-blue-500">98% FIT</div>
               <div className="px-3 py-1 bg-purple-500/10 border border-purple-500/20 rounded-lg text-[10px] font-bold text-purple-500">ELITE PACK</div>
            </div>

            <GlassCard className="!p-5">
               <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4">Proposed Architecture</h4>
               <p className="text-xs font-bold mb-1">Senior Product Designer</p>
               <p className="text-[10px] text-slate-500 dark:text-neutral-500 mb-4 ">$180k - $240k • San Francisco</p>
            ...
            <div className="space-y-4 pt-4">
               <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Upcoming Steps</h4>
               {[
                 { label: "Technical Sync", date: "Tomorrow, 10:00 AM" },
                 { label: "Portfolio Handover", date: "April 15, 2:00 PM" },
               ].map((step, i) => (
